...
public class SnmpPoller {
    ...
    private CommunityTarget<Address> createTarget(String ipAddress, int port, String communityString) {
    ...
    // 1. The Full Poll (Used only for Initial Discovery)
    public SnmpGetResult poll(String ipAddress, int port, String communityString) {
        try {
            CommunityTarget<Address> target = createTarget(ipAddress, port, communityString);
            PDU getPdu = new PDU();
            ...
            String sysName = null; String sysDescr = null; String nodeInfo = null;
            long uptime = 0; int cpuLoad = 0; long memAvail = 0; int diskUsage = 0; int temperature = 0; int congestion = 0;

            if (responsePdu.size() >= 9) {
                sysName = responsePdu.get(0).getVariable().toString();
                sysDescr = responsePdu.get(1).getVariable().toString();
                if (!responsePdu.get(2).getVariable().isException()) nodeInfo = responsePdu.get(2).getVariable().toString();
                if (!responsePdu.get(3).getVariable().isException()) uptime = responsePdu.get(3).getVariable().toLong();
                if (!responsePdu.get(4).getVariable().isException()) cpuLoad = responsePdu.get(4).getVariable().toInt();
                if (!responsePdu.get(5).getVariable().isException()) memAvail = responsePdu.get(5).getVariable().toLong();
    ...
    // 2. The Fast Heartbeat (Standard OIDs Only)
    public SnmpGetResult pollHeartbeat(String ipAddress, int port, String communityString) {
        try {
            CommunityTarget<Address> target = createTarget(ipAddress, port, communityString);
            PDU getPdu = new PDU();
            getPdu.add(new VariableBinding(new OID(SYS_NAME_OID)));
            ...
            String sysDescr = responsePdu.get(1).getVariable().toString();
            long uptime = 0;
            if (!responsePdu.get(2).getVariable().isException()) {
                uptime = responsePdu.get(2).getVariable().toLong();
            }

            return new SnmpGetResult(true, sysName, sysDescr, null, ipAddress, uptime, 0, 0, 0, 0, 0);
        } catch (Exception e) {
            return new SnmpGetResult(false, null, null, null, ipAddress, 0, 0, 0, 0, 0, 0);
        }
    }

    // 3. The Heavy Metrics Poll (Custom OIDs Only)
    public SnmpGetResult pollMetrics(String ipAddress, int port, String communityString) {
        try {
            CommunityTarget<Address> target = createTarget(ipAddress, port, communityString);
            PDU getPdu = new PDU();
            getPdu.add(new VariableBinding(new OID(CPU_LOAD_OID)));
            ...
            int cpuLoad = 0; long memAvail = 0; int diskUsage = 0; int temperature = 0; int congestion = 0;

            if (!responsePdu.get(0).getVariable().isException()) cpuLoad = responsePdu.get(0).getVariable().toInt();
            if (!responsePdu.get(1).getVariable().isException()) memAvail = responsePdu.get(1).getVariable().toLong();
            if (!responsePdu.get(2).getVariable().isException()) {
                try { diskUsage = Integer.parseInt(responsePdu.get(2).getVariable().toString().trim()); } catch(Exception ignored) {}
            }
            if (!responsePdu.get(3).getVariable().isException()) {
                try { temperature = Integer.parseInt(responsePdu.get(3).getVariable().toString().trim()); } catch(Exception ignored) {}
            }
            if (!responsePdu.get(4).getVariable().isException()) {
                try { congestion = Integer.parseInt(responsePdu.get(4).getVariable().toString().trim()); } catch(Exception ignored) {}
            }

            return new SnmpGetResult(true, null, null, null, ipAddress, 0, cpuLoad, memAvail, diskUsage, temperature, congestion);
        } catch (Exception e) {
            return new SnmpGetResult(false, null, null, null, ipAddress, 0, 0, 0, 0, 0, 0);
        }
    }
